  ...
  return (
    ...
    >
      ...
      (<button 
        ...
        onClick={handlerClick}>장바구니에 추가</button>)}
    </div>
  );
}

//# 
function Z_Example04() {
  // 상위 컴포넌트에서 전달할 함수 정의
  // : 매개변수 - 메뉴명
  // : 반환X - 콘솔 출력 `✔️ 상위 컴포넌트에서 처리: ${itemName} 장바구니 추가 완료`
  const handleAddToCard = (itemName: string): void => {
    console.log(`✔️ 상위 컴포넌트에서 처리: ${itemName} 장바구니 추가 완료`)
  }
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <p>아래는 props 조건에 따라 버튼이 다르게 렌더링 됩니다. (배열로 변환하여 작성해도 가능)</p>
