...
//     > 매번 API 호출하지 않고 한번 불러온 데이터를 전역에 저장하고 사용 (효율적)
...
interface GlobalState {
  ...
  // axios(Promise 기반 비동기) 반환타입 Promise<데이터타입>
...
// create 함수는 콜백함수를 가짐
// : 콜백함수는 set 설정함수를 매개변수로 받고, state 제네릭 타입을 반환
export const useGlobalStore = create<GlobalState>((set) => ({
  // 속성값과 함수를 정의
  // - 속성값: 초기값 할당
  // - 함수: 구조 정의 (해당 함수를 통해서 "속성값 업데이트"를 할수 있는 함수)
  categories: [],
  regions: [],
  isLoaded: false,
  ...
  fetchGlobalData: async () => {
    try {
      ...
      // });
      set({
        categories: ["COFFEE", "DESSERT", "DRINKS", "FOOD"],
        regions: ["SEOUL", "BUSAN", "DARJEON", "ULSAN"],
        isLoaded: true
      })
    } catch(e) {
      set({
