...
1) 단일 루트 노드
- '하나의 컴포넌트'가 단일 루트 노드만 반환
- 최상단의 루트 태그가 존재
- 주로 빈 Fragment <></> 사용

2) 태그 닫기
- 빈 태그(void 태그, 단일 태그) 사용 시 닫히는 <>에 /첨부
- input, hr, br, img 등
  ...
  return (
    <>
      ...
      */}
      ...
      {/* 
        요소에 이벤트 전달 시
        : 코드를 해석하는 과정에서 실행되지 않도록 '콜백 함수의 형태'로 전달
        - 익명함수 형태
        - 해당 이벤트가 발생해야만 내부 콜백함수가 실행
      */}
      <button onClick={()=>{handleClick('click')}}>클릭해주세요</button>
      <div style={divStyle} id='divElemnt' className='divClassElement'>
        div요소 - style 속성 내부에 스타일 객체를 삽입
      </div>
    </>
