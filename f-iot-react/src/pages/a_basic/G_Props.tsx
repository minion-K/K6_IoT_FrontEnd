...
function ChildComponent(props: { name: string }) {
  // props.name로 사용
  // : props는 readonly 속성이기 때문에 값의 재할당이 불가능
  // props.name = "홍길동";
...
function ChildComponent3({name}: Child2PropsType) {
  return (
    <div>
      안녕하세요 {name}님
    </div>
...
function MultiProps(props: {name: string; colorProps: string;}) {
  return (
    // props.name
    // props.colorProps
    // : 여러 개의 props가 전달되더라도 하나의 객체로 전달 받음
    // > 하나의 객체에 전달 받거나
    //    한 번에 각각의 요소 변수에 할당하는 구조 분해 할당도 가능
    // props.name = '읽기 전용 속성'; - 오류 발생
...
// === Props 정리 === //
// 1) 부모 -> 자식으로 가는 단방향 데이터 흐름
// 2) 자식은 props를 읽기만 가능 (readonly)
// 3) 상태 변경이 필요하면 부모로부터 콜백함수를 받아서 수행해야 함
...
function G_Props() {
  ...
  return (
    <>
      ...
      <MultiProps name='김유리' colorProps='lightblue'/>
