...
// - React 컴포넌트 스타일 유지 + 성능/유지보수성/가독성이 좋은 CSS 작성

//? 1. CSS Module 사용 방법
// 해당 컴포넌트에서 import 시 CSS 스타일이 객체로 인식
// : 내부의 스타일 속성 선택자를 객체의 속성처럼 사용
    ...
    <>
      ...
      <section className={gridStyles.dashboard}>
        ...
        >
          ...
          <div style={{padding: '16px', border: '1px solid #ddd'}}>5</div>  
        </div>
      </section>
      <hr />
      <section>
        {/*
          자식 요소들을 그리드 아이템으로 배치 
          1) repeat() 함수
            : 해당 열/행을 반복해서 생성
            EX) repeat(2, 1fr): 1의 비율을 가진 열을 2개 생성 (2등분)

          2) auto-fit vs auto-fill
            : 둘 다 남는 공간에 가능한 많은 칸을 자동으로 채워주는 역할
            > auto-fit
              : 빈 칸이 있으면 접기 -> 남는 공간을 기존 칸이 늘어져서 차지
            > auto-fill
              : 가능한 칸만큼 자리를 미리 잡아둠 (grid 칸이 계속 유지 되어 빈 칸이 있음)

        //? UI 상 카드들이 여백없이 꽉 차길 원한다면
            auto-fit을 더 많이 사용
          3) minmax()
            : 각 칸의 최소 크기와 최대 크기를 지정
        */}
