...
function Webcam() {
  ...
  //? useEffect: 마운트 시 실행될 로직 작성 (웹캠 작성)
  useEffect(() => {
    // 비동기 함수로 웹캠 설정
    async function setupWebcam() {
      ...
      // 비디오 ref가 현재 참조하는 DOM요소가 있다면
      if(videoRef.current) {
        // 해당 비디오 요소 소스로 스트림을 설정
  ...
  return (
    <div className='container'>
      <div className="camera-card">
        <h4 className='title'>카메라 웹</h4>
        {/* playsInline: 전체화면 전환없이 바로 재생 */}
        <video
          ref={videoRef}
          autoPlay
          playsInline
