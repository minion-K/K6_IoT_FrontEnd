...
type Reservation = {
  ...
  truckName: string; // 예약한 푸드트럭 이름
  timeSlot: string; // 예약 시간대(EX: 12:30 PM)
...
function Practice01() {
  ...
  const fetchReservations = async () => {
    ...
    try {
      ...
      // 실제 예약 구조로 변환(임시 매핑)
      // > slice(시작인덱스, 끝인덱스)까지의 얕은 복사본을 새로운 배열로 반환
      const mappedData: Reservation[] = data
        .slice(0, 5)
        .map((item: any, idx: number) => ({
          id: item.id, // mock.id
          customerName: item.name, // mock 사용자 이름
          truckName: `Truck-${idx + 1}`, // 트럭 이름 예시
          timeSlot: `12:${idx}0 PM`, // 시간대 예시 (12시부터 10분씩 차이)
    ...
    >
      ...
      <ul>
        {reservations.map((reservation) => (
          ...
          >
            <h4>
              {reservation.customerName} ({reservation.truckName})
            </h4>
            <p>시간대: {reservation.timeSlot}</p>
            <p>
              상태:{" "}
              <b
                style={{
