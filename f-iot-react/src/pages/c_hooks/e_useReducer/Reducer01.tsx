...
//   >> 해당 함수에 Action을 전달할 경우, 'reducer' 함수가 호출되어 새로운 상태 계산

// - reducer: 리듀서 함수(상태를 어떻게 변경할 지 정의한 함수)
//   >> useReducer의 인자로 전달되는 함수
//   >> 상태 변경 로직을 포함(switch, case)하고, 이전 상태와 액션 객체를 인자로 받아
//        , 새로운 상태를 반환
...
// 여러 상태가 하나의 이벤트로 함께 변경될 경우   : useReducer
...
function Reducer01() {
  ...
  return (
    <div>
      <h4>useState를 사용한 상태 관리</h4>
      <p>Count: {count}</p>
      <button onClick={handleIncrement}>증가</button>
      <button onClick={handleDecrement}>감소</button>

      <h4>useReducer를 사용한 상태 관리</h4>
