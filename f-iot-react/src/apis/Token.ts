...
// 2. 서버는 검증 후 Access Token + Refresh Token 두개 발급함
// 3. 클라이언트는 AccessToken을 저장 (보통 메모리/Zustand 등)
//    + Refresh Token은 HTTP Only 쿠키로 저장
// 4. 이 후 토큰이 필요한 API 요청 시 Authorization: Bearer <AccessToken> 형태로 보냄
// 5. 서버는 AccessToken을 검증하고 사용자 권한을 확인

//? AccessToken이 "짧은 만료 시간"을 가지는가?
//% 보안상 이유
...
//? Access Token이 짧을 경우의 불편함
//  : Access 토큰의 만료기간(10~15분) 마다 다시 로그인
//  - 해당 불편함을 줄이기 위해 Refresh Token이 존재

//? Refresh Token의 역할                                                         
//  : Access Token을 재발급할 수 있는 권한을 의미                               |   [Access Token]   
//  - 만료 수명이 길다                                                       |  짧음(15분 ~ 30분)
//  - 저장 위치    > HTTPOnly 쿠키(JS 접근 불가)                              |  메모리 / localStorage
//  - 노출 위험    > 낮음 (쿠키로만 전송)                                      |  높음(API 헤더에 직접 포함)
//  - 역할        > Access Token 재발급 용도                                |   API 요청 시 인증 증명
//  - 노출 시 위험 > Access Token 재발급만 가능, 즉시 사용 불가                  |   즉시 피해 가능