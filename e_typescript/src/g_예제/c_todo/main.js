...
var TaskManeger = /** @class */ (function () {
    ...
    // 3) 메서드
    // & 1. 할 일 추가(addTask)
    TaskManeger.prototype.addTask = function (content) {
        this.tasks.push({
            id: this.nextId,
            task: content,
            completed: false
        ...
        this.nextId++;
        this.updateTaskCount(); // 할 일 추가 업데이트
    };
    // & 2. 할 일 삭제(removeTask)
    ...
    TaskManeger.prototype.renderTasks = function (taskListId) {
        var _this = this;
        ...
        this.tasks.forEach(function (task) {
            ...
            // : A요소 안에 B요소를 삽입
            // - 내부의 첫 번째 요소보다 '전에' 앞선 배치 (textContent 내용보다 앞선 배치)
            li.insertBefore(checkbox, li.firstChild);
            taskList.appendChild(li);
        });
        this.updateTaskCount();
    };
    // & 4. 할 일 개수 업데이트(updateTaskCount)
    TaskManeger.prototype.updateTaskCount = function () {
...
document.addEventListener('DOMContentLoaded', function () {
    var taskManager = new TaskManeger();
    // DOM 요소 가져오기
