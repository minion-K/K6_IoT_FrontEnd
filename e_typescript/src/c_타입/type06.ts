...
// % +) 객체의 구조적 타이핑(덕 타이핑)
// 객체의 타입을 실제 값보다는 그 주소나 멤버에 의해 결정하는 방식
// - 객체의 형태가 유사(같다면), 같으면 타입으로 간주
...
// greet(p1); - Person 타입의 구조와 일치 X (Person 타입으로 취급 X)
...
let userData: UserDataType = {
  ...
  isStudent: false,
  hobby: '운동'
  // hobby: ['운동', '독서'] - 인덱스서명에 존재하지 않는 타입은 할당 불가
}

userData.email = 'minion._.';
