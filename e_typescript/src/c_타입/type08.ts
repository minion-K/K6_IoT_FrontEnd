...
// - 컴파일러에게 '이 데이터의 타입을 본인이 지정한 타입으로 간주'라는 지시
...
// -getElementById(), querySeletor() 등으로 HTML 요소를 가져올 때
//    return 받는 DOM 객체의 타입은 HTMLElement 타입
// +) 해당하는 선택자의 요소가 없을 경우 null 반환

// cf) HTMLElement 타입은 각 요소들이 가진 고유한 속성에 접근할 수 없음
//     >> 각 DOM 객체가 가지는 고유한 속성에 접근하기 위해서는 반드시 타입 단언이 필수
// EX) HTMLButtonElement로 단언 시 .disable 속성에 접근 가능
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // TS 컴파일러는 실시간 코드 변환 X
  // : 코드 수정 시 재컴파일 실행 필요
});

// % 2) JSON 문자열
// - 객체(parse) <-> JSON(stringfy)
// : JSON 문자열을 분석(parse)하여 반환되는 데이터
//   any 타입으로 간주

const jsonString = '{"name": "HGD", "age": 31}';
