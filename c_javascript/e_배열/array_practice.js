...
console.log(filterTen([3, 15, 23, 5, 11, 19, 10, 55, 60])); // [15, 23, 11, 19,10, 55, 60]
...
// : 배열을 순회하면서 max보다 큰 값이 있는 경우 max를 해당값으로 재할당
function findMax(array) {
  let max = array[0];
  let length = array.length;

  for(let i = 1; i < length; i++) {
    // 첫 번째 요소를 이미 max 변수에 할당
    // > 두 번째 요소부터 순회
    if(max < array[i]) {
      max = array[i]; // 재할당
    }
...
console.log(findMax([4, 7, 18, 3, 9]));