...
// : asynchronous (비동기적)
// >> 시간이 같지 않음
// - 특정 코드의 실행 완료를 기다리지 않고, 다음 코드를 실행하는 프로그래밍
...
// >> 느린 작업 끝나면 끝났음을 응답해주고 그 결과를 이어서 처리하는 구조
...
work(); // work() 작업이 종료되지 않으면 다름 콘솔창 출력 X: 동기 프로그래밍
console.log('동기 작업 완료 후 실행 될 코드 블록')
...
function asyncWork() {
  ...
  setTimeout(() => {
    ...
    const end = Date.now();
    console.log(end - start + "ms");
  }, 0); // 지연시간 0: 최소 지연 시간(브라우저마다 약 4ms 정도의 지연시간이 있음)
