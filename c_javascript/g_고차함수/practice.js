...
// ! 1번 문제
// 다음 배열을 조건에 따라 작성하고 실행 했을 때의 결과를 확인
...
// 각 요소에 10을 더하고 홀수값만을 새로운 배열에 담아 출력
const filteredNumbers = numbers.map(number => number + 10).filter(number => number % 2 === 1);
...
const filteredWords = (word, substring) => {
  ...
  //    >> 배열, 문자열등에서 데이터 인자값이 포함되어있는지를 검사(boolean 반환)
  return word.filter(item => item.includes(substring));
}
