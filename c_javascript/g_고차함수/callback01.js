...
// cf) JS의 함수
function funcName1() {}         // 함수 선언식
const funcName2 = function() {} // 함수 표현식
const funcNam3 = () => {}       // 화살표 함수
...
function callback1(callbackFunc) { // print 함수가 인자로 전달되면 callbackFunc 매개변수명으로 사용됨
  for(let i = 0; i < 3; i++) {
    callbackFunc(i + 1); // i값은 print함수의 index 매개변수로 전달
  }
}
...
const evenFunc = evenNum => console.log(`${evenNum}은(는) 짝수입니다.`);
const oddFunc = oddNum => console.log(`${oddNum}은(는) 홀수입니다.`);
...
callback2(3, evenFunc, oddFunc); // 3은(는) 짝수입니다.
callback2(4, evenFunc, oddFunc); // 4은(는) 홀수입니다.