...
function addTodo(content) { // 함수 호출 시 인자로 할 일의 내용을 전달 받음
  ...
  // 다음 할 일 추가 시 사용할 고유 ID 값(전역변수) 증가
  nextId++;
...
// : 수정 하고자 하는 특정 할 일의 ID를 매개변수로 전달받아 '완료 상태 전환(토글)'
function toggleTodo(id) {
  ...
  todos = todos.map(todo => {
    ...
    if(todo.id === id) {
      ...
      // ? +) 객체의 속성 변경 
      // 객체명.속성키 = 속성값;
      // - 해당 속성키가 존재하는 경우: 객체 속성값에 대한 재할당
...
// >> 배열 내부 요소에서 제거(요소 개수 변화 >> filter)
function deleteTodo(id) {
  const idx = todos.findIndex(todo => todo.id === id); // 삭제할 요소의 인덱스 번호 반환
  if(idx === -1) {
