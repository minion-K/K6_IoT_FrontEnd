...
// 2) name - 상품 이름 (문자)
// 3) price - 상품 가격 (숫자)
// 4) quantity - 사용자가 선택한 해당 상품 수량 (숫자)
...
function addToCart(id, name, price, quantity) {
  ...
  if(index > -1) {
    // 상품이 이미 장바구니에 존재 하는 경우
    cart[index].quantity += quantity; // 기존의 수량에 추가된 수량을 더함
  } else {
    // 상품이 장바구니에 없을 경우
    // let newItem = {
    //   id: id,
...
// ? 3. 특정 상품의 수량을 변경
function updateQauntity(id, quantity) {
  const idx = cart.findIndex(item => item.id === id);

  // 상품이 존재하고 변경할 수량이 0 보다 커야만 상품 업데이트
  if(idx > -1 && quantity > 0) {
    // 업데이트
