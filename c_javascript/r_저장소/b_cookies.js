...
// ! 쿠키의 구성 요소 (구조)
// - 이름(name): 쿠키를 구별하는 식별자
// - 값(value): 쿠키에 저장할 실제 데이터 (문자열)
// - 만료 날짜(expires): 쿠키의 유효시간 (없으면 세션 종료 시 삭제)
...
// - 쿠키이름, 쿠키값 (필수)
// - 나머지 (선택)
document.cookie = "username=kmh; path=/ "
...
// : 브라우저가 스스로 만료를 관리
// - 자동 삭제(즉시 사라짐) X
// >> 브라우저가 쿠키에 접근하려고 하거나 갱신할 때 정리
// >> 만료 시간 후에도 메모리 상에는 잠시 남아있을 수 있음

// % 3) 쿠키 읽기
// : 쿠키는 하나의 문자열로 반환
console.log(document.cookie);
// userEmail; username=kmh; userEmail=qwe123
// >> 문자열이기 때문에 직접 파싱
